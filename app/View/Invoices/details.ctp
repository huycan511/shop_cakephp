<div class="d-sm-flex align-items-center justify-content-between mb-4">
  ...
  <button class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" id="export_pdf" data-id="<?php echo $invoice['Invoice']['id']?>"> Generate Report</button>
</div>
...
<?php }else if(($invoice['Invoice']['status']== 0) && ($this->Session->read('id_store')==$invoice['Invoice']['id_send']) && ($invoice['Invoice']['type']==4)){ ?>
  ...
  <button class="btn btn-danger d-none cancel_order" data-store="<?php echo $this->Session->read('id_store')?>" data-id="<?php echo $invoice['Invoice']['id']?>">Cancel Order</button>
  ...
  <div class="card-body">
    <table class="table table_product" border="0" cellspacing="0" cellpadding="0">
        <tbody>
            ...
            <tr>
                <th width="20%"><?php if($invoice['Invoice']['type']==1){echo 'Supplier';}else{echo 'Store send';}?></th>
                <td width="20%">
            ...
            <tr>
                ...
                <td width="20%">
                  <span><?php if(($invoice['Invoice']['type']==1) || ($invoice['Invoice']['type']==2)){
                    ...
                    if($invoice['Invoice']['id_receive']==null){
                      echo 'Khách vãng lai';
                    }else{
                      echo $invoice['Invoice']['id_receive'];
                    }
                  }
                  ?></span>
                </td>
            ...
            <?php if($invoice['Invoice']['type']==4){?>
              ...
              <tr>
                  <th width="20%">Note</th>
                  <td width="20%">
                    <span><?php if($invoice['Invoice']['note']==''){echo 'Null';}else{echo $invoice['Invoice']['note'];}?></span>
                  </td>
              </tr>
            <?php }?>
            <tr>
