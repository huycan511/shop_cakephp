					...
					<div class="details col-md-6">
						...
						<div class="action">
							<button data-izimodal-open="<?php if (!$this->Session->read('id_user')) {
																						echo "#modal-custom";
																					} ?>" class="add-to-cart btn btn-default add_cart" onclick="<?php if ($this->Session->read('id_user')) {
																																				echo 'addcart($(this))';
																																			} ?>" data-user="<?php echo $this->Session->read('id_user') ?>" data-product="<?php echo $product['Product']['id'] ?>" type="button">Add to cart</button>
							<button data-price="<?php echo $product['Product']['price'];?>" style="font-weight: bold;" class="btn btn-default buy_now_product_page" type="button">BUY NOW</button>
							<button class="like btn btn-default <?php if ($this->Session->read('id_user')) {
																										echo 'like_product';
																									} ?>" data-product="<?php echo $product['Product']['id'] ?>" type="button"><span class="fa fa-heart"></span></button>
						</div>
					</div>
				</div>
			</div>
	...
	<div class="productinfo-tab">
		...
		<div class="tab-content">
			...
			<div class="tab-pane ui comments" id="tab-review">
				<div id="rateYo" class="<?php if (!$hasBuy) {
																	echo 'd-none';
																} ?>" data-id="<?php echo $product['Product']['id'] ?>" data-rated="<?php if (isset($myRate)) {
																																																																			echo $myRate;
																																																																		} else {
																																																																			echo '0';
																																																																		} ?>" style="padding: 0 0 !important; margin-bottom: 10px;"></div>
				...
				<?php for ($i = 0; $i < count($related); $i++) { ?>
					<div class="item">
						<div class="product-thumb transition">
							<div class="image product-imageblock"> <a href="<?php echo FIELD . '/home/product/' . $related[$i]['Product']['id'] ?>"> <img src="<?php echo FIELD; ?>/app/webroot/img/product/<?php $img = explode(",", $related[$i]['Product']['image']);
...
<script type="text/javascript">
	...
	function reloadShowComment($sum_comment, $option_number_show) {
		...
		if (parseInt($sum_comment) > parseInt($option_number_show)) {
			$('.see_all_comment').text('See All Comment').attr({
				onclick: 'showAllCommet()'
			});
		}
	};
